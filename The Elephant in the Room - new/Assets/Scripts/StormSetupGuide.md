...
1. Call `StormController.Instance.ActivateStorm()` to begin the storm
2. Call `StormController.Instance.DeactivateStorm()` to end the storm
3. The controller will manage all subsystems automatically:
...
### 8.3 Manual Inspector Controls
The StormController includes custom inspector buttons for easy testing and debugging:

1. **Activate Storm**: Manually starts the storm system
2. **Deactivate Storm**: Manually stops the storm system  
3. **Toggle Storm**: Switches between active/inactive states
4. **Current Status**: Shows real-time storm state (ACTIVE/INACTIVE)

To use these controls:
1. Select the StormController GameObject in the Hierarchy
2. In the Inspector, scroll down to the "Manual Controls" section
3. Click any of the buttons to control the storm
4. The current status is displayed with color coding (green = active, red = inactive)

These controls are useful for:
- Testing storm effects during development
- Debugging individual storm components
- Manual weather control for gameplay testing
- Verifying all storm subsystems are working correctly



## 9. Performance Optimization

### 9.1 Rain System Optimization
- Limit rain particles to 50m around the camera
- Use LOD system for particle quality at distance
- Pool splash sub-emitters

### 9.2 Wind Effects Optimization
- Only apply wind effects to visible foliage
- Use instancing for similar objects
- Limit wind calculations to nearby objects

### 9.3 Audio Optimization
